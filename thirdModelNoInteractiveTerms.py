...
# Quadratic expansion (includes linear + squared + interactions)
poly = PolynomialFeatures(degree=2, include_bias=False)
X_poly_full = poly.fit_transform(X)
feature_names_full = poly.get_feature_names_out(X.columns)

# Remove interaction terms (keep only linear and squared)
filtered_features = []
filtered_columns = []

for i, name in enumerate(feature_names_full):
    if '^2' in name or re.fullmatch(r"^[A-Za-z]+$", name):  # linear or squared
        filtered_features.append(name)
        filtered_columns.append(X_poly_full[:, i])

# Rebuild X_poly and feature_names
X_poly = np.column_stack(filtered_columns)
