...
# Apply full quadratic expansion to all features
poly = PolynomialFeatures(degree=2, include_bias=False)
X_poly = poly.fit_transform(X)

# Get names of all expanded features
...
# Recover full feature names again in case they were reassigned
# Use same poly used during fit
feature_names = poly.get_feature_names_out(X.columns).tolist()

# Verify shape
if len(feature_names) != len(model.coef_):
    raise ValueError(f"Feature name mismatch: {len(feature_names)} names vs {len(model.coef_)} coefficients")

# Non-normalized export
...
output_path = os.path.join("FinalProject", "exported_model_coefficients_raw.csv")
export_df.to_csv(output_path, index=False)

print(f"\n📤 Exported raw coefficients to: {output_path}")
